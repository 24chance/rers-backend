...
let BootstrapService = class BootstrapService {
    ...
    async seed(dto) {
        const existingAdmin = await this.database.user.findFirst({
            where: { role: { name: user_role_enum_1.UserRole.SYSTEM_ADMIN } },
        });
        if (existingAdmin) {
            throw new common_1.ConflictException('System has already been bootstrapped. This endpoint is disabled.');
        }
        const seededRoles = [];
        ...
        const adminRole = await this.database.role.findUnique({
            where: { name: user_role_enum_1.UserRole.SYSTEM_ADMIN },
        });
